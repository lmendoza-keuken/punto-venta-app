...
class PrinterLocalDataSourceImpl implements PrinterLocalDataSource {
  ...
  Future<PrinterConfig> getPrinterConfig() async {
    ...
    return PrinterConfig(
      ip: data['ip']?.toString() ?? '192.168.0.230',
      port: (data['port'] is int) ? data['port'] as int : int.tryParse(data['port']?.toString() ?? '') ?? 9100,
      timeout: (data['timeout'] is int) ? data['timeout'] as int : int.tryParse(data['timeout']?.toString() ?? '') ?? 10000,
    );
  }
